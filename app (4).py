...
# Disable Streamlit file watcher to avoid inotify errors
...
def send_email_notification(receiver_email, complaint_id, title, new_status):
    if not ADMIN_EMAIL or not ADMIN_APP_PASSWORD:
        st.warning("⚠️ Admin email not configured. Skipping email notification.")
        return
    try:
        msg = MIMEMultipart()
    ...
    if admin_password_input != ADMIN_PASSWORD:
        ...
        if len(df) == 0:
            ...
            if st.button("Update Status"):
                ...
                st.success(f"Complaint #{selected_id} marked as '{new_status}' and email sent.")
