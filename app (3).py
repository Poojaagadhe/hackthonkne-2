...
# Geolocator
# ----------------------------
geolocator = Nominatim(user_agent="civic_issue_tracker_app")
...
if page == "📋 Report Issue":
    ...
    st.markdown("### Select Location on Map or Enter Manually")
    ...
    if map_click and map_click.get("last_clicked"):
        ...
        st.success(f"📍 Location selected on map: ({lat:.4f}, {lon:.4f})")

    manual_location = st.text_input("Or Enter Location (City, State, Address)")
    if manual_location and (lat is None or lon is None):
        try:
            location = geolocator.geocode(manual_location)
            if location:
                lat = location.latitude
                lon = location.longitude
                st.success(f"📍 Manual location geocoded: ({lat:.4f}, {lon:.4f})")
            else:
                st.warning("❌ Could not geocode manual location.")
        except Exception as e:
