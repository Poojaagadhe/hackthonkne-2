...
geolocator = Nominatim(user_agent="civic_issue_tracker")
...
if os.path.exists(DATA_FILE):
    ...
    df = pd.DataFrame(columns=[
        "id", "title", "description", "email", "latitude", "longitude", "address",
...
if page == "📋 Report Issue":
    ...
    if map_click and map_click.get("last_clicked"):
        ...
        st.info(f"🏠 Approx. Address: {address}")
    ...
    if len(resolved_df) == 0:
        ...
        for _, row in resolved_df.iterrows():
            if pd.notna(row["latitude"]) and pd.notna(row["longitude"]):
                popup_text = f"<b>{row['title']}</b><br>{row['description']}<br><i>{row['timestamp']}</i><br><i>{row['address']}</i>"
                folium.Marker(
        ...
        st.dataframe(resolved_df[["id", "title", "timestamp", "address"]])
